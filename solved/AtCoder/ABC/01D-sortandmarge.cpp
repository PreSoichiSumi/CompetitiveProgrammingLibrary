...
int main() {
    ...
    if (n == 1) {
        cout << setw(4) << setfill('0') << memo[0].first << "-";
        cout << setw(4) << setfill('0') << memo[0].second << endl;
    } else {
        //res.push_back(memo[0]);
        P p=memo[0];
        for (int i = 1; i < memo.size(); ++i) {
            //P &last = res.back();
            if (memo[i].first <= p.second) {//重なっている
                //last=MP(last.first,max(last.second,memo[i].second));
                p=MP(p.first,max(p.second,memo[i].second));
            } else {
                //res.push_back(memo[i]);
                cout << setw(4) << setfill('0') << p.first << "-";
                cout << setw(4) << setfill('0') << p.second << endl;
                p=memo[i];
            }
        }
        cout << setw(4) << setfill('0') << p.first << "-";
